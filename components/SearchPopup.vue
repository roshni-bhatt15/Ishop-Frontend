  ...
  <div class="search-popup">
    <div class="sb popup-inner">
      <div class="pop-over-content">
        <div class="spinner-wrapper flex layer-white" v-if="fetchingData">
          <spinner :radius="100" />
        </div>
        ...
        <div v-else-if="matchedResult">
          ...
          <div v-if="categories.length" class="mb-15">
            <h4 class="bold">
            ...
            <div class="search-section category-wrapper">
              ...
              >
                <div class="img-wrapper">
                  <ImageLazy
                    :lazy-src="utils.getImageURL(value.image)"
                    :title="value.title"
                    :alt="value.title"
                    height="50"
                    width="50"
        ...
        <div v-else>
          <h4>
            {{ $t("searchPopup.nothingFound") }} "<span
              class="color-primary semi-bold"
              >{{ props.searchedText }}</span
            >"
          </h4>
        </div>
      </div>
...
const { fetchSearchedSuggestion, updateSearch, setSearchedSuggestion } =
  listingStore;
...
const matchedResult = computed(() => {
  return (
    products.value.length || suggested.value.length || categories.value.length
  );
});
