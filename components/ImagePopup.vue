  ...
  <div :class="{ stacked: stackedOnResponsive }" class="image-popup-wrapper">
    ...
    <div class="image-popup">
      <div v-if="stackedOnResponsive" class="image-popup-icons">
        <button class="left-btn fav-btn" @click.prevent="addToWishlist">
          <i :class="isFavourited" class="m-0 icon" />
        </button>
        <button class="right-btn share-btn" @click="toggleSharePop">
        ...
        <template v-slot:content>
          <li v-for="(value, index) in imageList" :key="index">
            <img
              ...
              :alt="$t('invent.si')"
              :data-src="utils.getImageURL(value.image)"
            />
          </li>
        ...
        <template v-slot:bullet-area>
          ...
          >
            <img
              ...
              :alt="$t('invent.si')"
              :src="utils.getThumbImageURL(value.image)"
            />
          </div>
          ...
          >
            <img
              class="embla-thumbs__slide__number img-handler"
              :alt="$t('invent.si')"
              :src="utils.getImageURL(videoThumb)"
            />
...
const { onChange, onFirstImageLoad } = useSliderHelper(
  activeSlide,
  sliderLoaded,
);
