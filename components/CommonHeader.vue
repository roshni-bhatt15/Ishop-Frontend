        ...
        <div ref="headerWrapperRef" class="wrap flex sided">
          <div class="left-side wrap flex gap-10">
            ...
            <template v-if="phone">
              ...
              <a :href="`tel:${phone}`" class="flex gap-5">
                ...
                <span
                  ><span class="hide-md">{{ $t("home.helpline") }}</span>
                  {{ phone }}</span
                >
              </a>
            </template>
          ...
          <div class="flex right-side text-upper">
            <div class="flex gap-5" v-if="!isLoggedIn">
              ...
              <nuxt-link to="/login" @click="beforeLogin" class="flex gap-5">
      ...
      <div class="container-fluid flex pos-rel">
        <div class="left-area">
          <nuxt-link to="/" class="logo">
            <img
              :src="utils.getImageURL(site_setting.header_logo)"
        ...
        <div class="right-area flex gap-15 right">
          <div class="pos-rel" v-outside-click="closeDropdown">
            ...
            <div class="dropdown" :class="{ active: showDropdown }">
              <nuxt-link to="/user/orders" @click.prevent="closeDropdown">
                {{ $t("header.orders") }}
              </nuxt-link>
...
const { site_setting, setting, topBanner, headerLinks } =
...
const selectedLanguage = (data) => {
  setLocale(data.key);
  document.cookie =
    "currentLanguage=" + data.key + "; path=/; max-age=" + 365 * 60 * 60 * 24;
  location.reload();
};
...
const search = () => {
  if (
    searchedText.value &&
    (searchedText.value !== searched.value || route.name !== "search")
  ) {
