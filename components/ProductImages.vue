<template>
  <div class="detail-image shimmer-wrapper">
    <template v-if="mainImgLoaded">
      ...
      >
        <ImageZoom
          :image="utils.getThumbImageURL(selectedImage)"
          :zoom-image="utils.getImageURL(selectedImage)"
          @click.prevent="imagePopupOpen()"
        />

        <ImageSlider class="thumb-slider">
          <template v-slot:content>
            <li
              v-for="(value, index) in productImageList"
              :key="index"
            ...
            >
              <ImageLazy
                :class="
                  activeIndex === productImageList.length ? 'choosed-thumb' : ''
                "
                class="responsive-image"
    ...
    <template v-else>
      <div class="pleceholder-zoomer-base-container center-text">
        <img
          class="preload-img"
          :src="utils.getThumbImageURL(mainImage)"
          :alt="title"
          height="100"
          width="100"
        />
