import { p as prepareGetUrl } from './server.mjs';

const addressHelper = {
  data() {
    return {
      fetchingAddressData: false
    };
  },
  methods: {
    async addressAction() {
      var _a, _b;
      if (this.addressData.email && this.addressData.city && this.addressData.phone && this.addressData.name && this.addressData.zip && this.addressData.country && this.addressData.address_1) {
        this.submittingAddressData = true;
        const data = await this.postRequest({
          api: "userAddressAction",
          params: {
            ...this.addressData,
            ...{
              user_token: await this.getUserToken()
            }
          }
        });
        if ((data == null ? void 0 : data.status) === 200) {
          this.hasAddressErrors = false;
          this.updateAddress(data == null ? void 0 : data.data);
          this.setToastMessage(data.message);
        } else if ((data == null ? void 0 : data.status) === 201) {
          this.setToastError((_b = (_a = data.data) == null ? void 0 : _a.form) == null ? void 0 : _b.join(", "));
        } else {
          this.hasError(data);
        }
        this.submittingAddressData = false;
      } else {
        this.hasAddressErrors = true;
      }
    },
    async deleting(address) {
      var _a, _b;
      if (confirm(this.$t("cartProductTile.deleteAlert"))) {
        this.ajaxDeleting = address.id;
        const data = await this.deleteRequest({
          api: "userAddressDelete",
          params: `/${address.id}?${prepareGetUrl({
            user_token: await this.getUserToken()
          })}`
        });
        if ((data == null ? void 0 : data.status) === 200) {
          this.setToastMessage(data.message);
          await this.fetchingData();
        } else {
          this.setToastError((_b = (_a = data == null ? void 0 : data.data) == null ? void 0 : _a.form) == null ? void 0 : _b.join(", "));
        }
        this.ajaxDeleting = 0;
      }
    },
    formatAddress(obj) {
      let addArr = [];
      addArr.push(obj.address_1);
      if (obj.address_2) {
        addArr.push(obj.address_2);
      }
      addArr.push(obj.city + "-" + obj.zip);
      if (this.countryList[obj.country]) {
        const country = this.countryList[obj.country];
        if (country.states[obj.state]) {
          addArr.push(country.states[obj.state].name);
        }
        addArr.push(country.name);
      }
      let addressStr = addArr.filter(function(el) {
        return el != null;
      }).join(", ");
      return addressStr;
    },
    async fetchingData() {
      this.fetchingAddressData = true;
      setTimeout(async () => {
        this.settingRouteParam();
        const data = await this.getRequest({
          api: "userAddressAll",
          params: `?${prepareGetUrl({
            time_zone: this.timeZone,
            order_by: this.orderBy,
            type: this.orderByType,
            page: this.page,
            user_token: await this.getUserToken(),
            q: this.search
          })}`
        });
        if ((data == null ? void 0 : data.status) !== 200) {
          this.hasError(data);
        } else {
          this.setAllAddress(data == null ? void 0 : data.data);
        }
        this.fetchingAddressData = false;
      }, 100);
    }
  }
};

export { addressHelper as a };
//# sourceMappingURL=addressHelper-BBaujjhf.mjs.map
